...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    /* Kishor */
    UIView *viewImgSub = view.subviews[4];
    UIView *viewContentSub = view.subviews[2];
    if ([self alphaForItemWithOffset:offset] > 0.8) {
        viewImgSub.hidden = YES;
        viewContentSub.hidden = YES;
    }else {
        viewImgSub.hidden = NO;
        viewContentSub.hidden = NO;
    }
    /* Kishor */
    
    view.superview.layer.opacity = 1.0;
