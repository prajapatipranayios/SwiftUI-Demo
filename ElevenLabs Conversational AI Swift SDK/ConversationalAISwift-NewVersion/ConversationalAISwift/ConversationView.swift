...
struct ConversationalAIExampleView: View {
    ...
    private var languagePicker: some View {
        Group {
            Picker(selection: $selectedLang) {
                ...
                HStack(spacing: 8) {
                    ...
                    Text("   \(selectedLang?.langName ?? "Select Language")")
                        .font(.custom("Rubik-Regular", size: 13))
                        .foregroundColor(objViewModel.isConnected ? .black : .black)
                        .padding(.leading, 8)
                }
                .padding(8)
            ...
            .foregroundStyle(objViewModel.isConnected ? Color.gray : Color.white)
            .tint(objViewModel.isConnected ? Color.gray : Color.white)
            .pickerStyle(.menu)
